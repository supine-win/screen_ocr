...
2. **OCR识别**: 使用PaddleOCR进行中文文字识别，支持字段映射配置
3. **HTTP服务**: 提供健康检查和OCR识别API接口
4. **截图存储**: 按年/月/日文件夹自动存储截图
5. **配置管理**: JSON格式配置文件，支持实时更新
...
2. **摄像头权限**: 确保摄像头权限已开启
3. **网络安全**: HTTP服务默认监听所有网络接口，注意安全设置
4. **存储管理**: 截图文件按年/月/日自动分类存储
5. **自动清理**: 支持自动清理过期截图文件
6. **跨平台**: 支持macOS开发，Windows部署
7. **离线运行**: 打包后完全离线，无需Python环境
